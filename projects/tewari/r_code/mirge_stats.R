...
keys = read_csv("tewari/mirge/sample_fn_key.txt", col_names = c("fn", "inside")) %>% 
    mutate(fn = gsub("_isomiRs.gff", "", fn))
mirge_stats = read_csv("tewari/mirge/stats/mirtop_stats.txt", skip = 1) %>% 
...
ggplot(mirge_stats %>% filter(grepl("_count", category),
    ...
    ggsave("figures/stats/mirge_count.pdf")
...
ggplot(mirge_stats %>% filter(grepl("_sum", category),
    ...
    geom_bar(stat = "identity", position = "dodge") +
    facet_wrap(~category, scales = "free_y", nrow=4) +
    ggsave("figures/stats/mirge_sum.pdf")
